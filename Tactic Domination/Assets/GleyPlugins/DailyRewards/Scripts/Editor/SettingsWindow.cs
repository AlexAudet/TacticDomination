    ...
    public class SettingsWindow : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            for (int i = 0; i < localCalendarDays.Count; i++)
            {
                ...
                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.LabelField("Reward Type", GUILayout.Width(205));
                localCalendarDays[i].rewardType = (RewardType)EditorGUILayout.EnumFlagsField(localCalendarDays[i].rewardType);
                EditorGUILayout.EndHorizontal();

                if(localCalendarDays[i].rewardType == RewardType.Minion)
                {
                    EditorGUILayout.BeginHorizontal();
                    EditorGUILayout.LabelField("Minion Key", GUILayout.Width(205));
                    localCalendarDays[i].key = EditorGUILayout.TextField(localCalendarDays[i].key);
                    EditorGUILayout.EndHorizontal();
                }
